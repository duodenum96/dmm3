...
function ple_slidingwindow_parallel(x; flow=0.001, fhigh=0.01, windowsize=10000, fs=30, siegel=false)
    n = length(x)
    ...
    Threads.@threads for i in 1:(n-windowsize+1)
        ...
        slide_ple[i] = calc_ple(x[window[1]:window[2]]; flow=flow, fhigh=fhigh, fs=fs, siegel=siegel)
    end

    return slide_ple
end

function ple_slidingwindow_nooverlap(x; flow=0.001, fhigh=0.01, windowsize=10000, fs=30, siegel=false)
    n = length(x)
    n_window = n ÷ windowsize
    slide_ple = zeros(n_window)
...
function sl_overlap_indices(n, windowsize, overlap)
    nwindow = n ÷ windowsize # (alternatively, div(n, windowsize))
    convert.(
        AbstractArray{Int}, 
        [[1 + (1-overlap)*windowsize*(i-1), (1-overlap)*windowsize*(i-1) + windowsize] for i in 1:nwindow]
    )
end

function ple_slidingwindow_overlap(x; flow=0.001, fhigh=0.01, windowsize=1000, overlap=0.5, fs=30, siegel=false)
    n = length(x)
    idx = sl_overlap_indices(n, windowsize, overlap)
    n_idx = length(idx)
    slide_ple = zeros(n_idx)

    Threads.@threads for i in 1:n_idx
        window = idx[i]
        slide_ple[i] = calc_ple(x[window[1]:window[2]]; flow=flow, fhigh=fhigh, fs=fs, siegel=siegel)
    end
    return slide_ple
